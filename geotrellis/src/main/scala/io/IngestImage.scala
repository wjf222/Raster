...
object IngestImage {
  ...
  //  val inputPath = "data/mer.tif"
  //  val outputPath = "data/catalog"

  //  val outputPath = ""
  def main(args: Array[String]): Unit = {
    // Setup Spark to use Kryo serializer.
    var maskedPath = "hdfs://namenode:8020" + args(0)
    var resultPath = "hdfs://namenode:8020" + args(1)
    val conf =
      new SparkConf()
//        .setMaster("local[*]")
        .setAppName("Geotrellis nvdi")
        .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
        .set("spark.kryo.registrator", "geotrellis.spark.io.kryo.KryoRegistrator")

    val sc = new SparkContext(conf)
    try {
      run(sc, maskedPath, resultPath)
...
//      .reduceByKey(_.localMax(_))
    dont.saveAsObjectFile(outputPath)
